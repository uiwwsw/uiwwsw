import { getEssaysUseCase, getEssayUseCase } from '../application';
import type { Essay, KakaoStoryFeed } from '../domain/essay';
import type { EssayService } from '../service';

export class EssayAdapter implements EssayService {
	//https://developers.kakao.com/tool/rest-api/open/get/v1-api-story-mystories
	...
	readonly essays = this.kakaoStoryFeeds.map((x) => ({
		id: x.id,
		url: x.url,
		content: x.content,
	...
	getEssay(repositoryUrl: string): Essay | undefined {
		return getEssayUseCase(this.essays)(repositoryUrl);
	}
}
