...
- **[롱폼리뷰](https://longform-review.duckdns.org/)** · <sub>개발 중</sub><br>
  별점 대신 구체적인 이용 경험과 검토 근거로 리뷰를 읽고 비교하는 플랫폼.
- **[JSONSURVEY](https://jsonsurvey.duckdns.org/)** · <sub>개발 중</sub><br>
  JSON으로 질문·분기·결과를 정의하고, 답변에 반응하는 설문을 제작·공유하는 플랫폼.
- **베디(Be:D)** · <sub>앱 출시 준비</sub><br>
  좋은 운전을 칭찬으로 연결하는 앱. 주행 중 조작을 제한하고 정차 후 참여하도록 설계합니다.

## 반복되는 문제는 도구로

**[react-query-helper](https://github.com/uiwwsw/react-query-helper)** · 반복되는 API 연결을 자동화하는 CLI. TypeScript 함수에서 TanStack Query 옵션을 생성합니다.<br>
...
**[test-mode](https://github.com/uiwwsw/test-mode)** · 실패·지연 상태를 재현하는 API mock/patch 도구. 개발·QA·디자인이 같은 화면에서 문제를 확인합니다.<br>
...
**[virtual-keyboard](https://github.com/uiwwsw/virtual-keyboard)** · 브라우저 IME와 분리된 입력 모델로 한글 조합을 직접 제어하는 React 컴포넌트.<br>
